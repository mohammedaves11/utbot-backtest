...
def load_mt5(symbol:        str = 'XAUUSD.t',
    ...
    timeframe_str : str — 'M1','M5','M15','M30','H1','H4','D1','W1'
                          (also accepts lowercase: '1m','5m','15m','1H','4H','1D')
    from_date     : str or datetime — start date, e.g. '2022-01-01'
    to_date       : str or datetime — end date,   e.g. '2024-12-31'
                                      pass 'today' to use today's date
    ...
    Examples
    --------
    df = load_mt5('XAUUSD', 'M15', from_date='2022-01-01', to_date='2024-12-31')
    df = load_mt5('XAUUSD', 'H1',  from_date='2023-06-01', to_date='today')
    """
    ...
    def _parse(d):
        ...
        if s == 'today':
            today = dt.utcnow()
            return dt(today.year, today.month, today.day, 23, 59, 59)
        ...
        for fmt in ('%Y-%m-%d', '%Y/%m/%d', '%d-%m-%Y', '%d/%m/%Y'):
            try:
                ...
                continue
        raise ValueError(
            f"Cannot parse date '{d}'. Use 'YYYY-MM-DD' format, "
            f"a datetime object, or 'today'."
        )
...
def load_data(source:    str = 'csv',
    ...
    from_date : start date — str 'YYYY-MM-DD', datetime, or None  (MT5 only)
    to_date   : end date   — str 'YYYY-MM-DD', datetime, or 'today' (MT5 only)
    **csv_kwargs : forwarded to load_csv()  (time_col, sep, open_col, …)
    ...
    # From MT5 with date range
    df = load_data('mt5', symbol='XAUUSD', timeframe='M15',
                   from_date='2022-01-01', to_date='2024-12-31')

    # MT5 up to today
    df = load_data('mt5', symbol='XAUUSD', timeframe='M15',
                   from_date='2023-01-01', to_date='today')
    """
    source = source.lower()
    if source == 'csv':
        if not csv_path:
        ...
        return load_mt5(symbol, timeframe, from_date, to_date)
    else:
...
def calc_metrics(trades: list, equity_curve: pd.Series,
    ...
    # Max Drawdown (calculated on balance-offset curve for correct % figure)
    bal_curve = equity_curve + initial_balance
    peak      = bal_curve.cummax()
    dd        = bal_curve - peak
    mdd       = dd.min()
    idx_mdd   = dd.values.argmin()
    worst     = peak.iloc[idx_mdd] if len(peak) > 0 else initial_balance
...
def generate_report(trades:          list,
                    ...
                    symbol:          str   = 'XAUUSD',
                    params:          dict  = None,
                    initial_balance: float = 100_000.0) -> str:
    """
    ...
    initial_balance : starting account equity in USD (default 100,000)
                      The equity curve is displayed as: initial_balance + cumulative P&L

    Returns
    -------
    output_path : str — same as the input argument
    """
    # Offset equity curve so it starts at initial_balance
    equity_curve = equity_curve + initial_balance
    try:
        import matplotlib
        matplotlib.use('Agg')
