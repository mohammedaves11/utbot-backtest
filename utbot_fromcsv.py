...
# ─────────────────────────────────────────────
#  LOAD FROM MT5 CSV (alternative)
# ─────────────────────────────────────────────
def load_from_csv(filepath):
    """
    Use this if you export data directly from MT5.
    In MT5: File -> Export -> Save as CSV
    Required columns: datetime, open, high, low, close, volume
    """
    print(f"Loading CSV: {filepath}")
    df = pd.read_csv("XAUUSD_M15_TillDate.csv", parse_dates=['datetime'], index_col='datetime')
    df.columns = [c.lower() for c in df.columns]
    if 'volume' not in df.columns:
        df['volume'] = 0
    df = df[['open', 'high', 'low', 'close', 'volume']].copy()
    df.dropna(inplace=True)
    print(f"CSV loaded: {len(df)} bars")
...
def main():
    ...
    df = load_from_csv("XAUUSD_M15_TillDate.csv")
    if df.empty:
        mt5.shutdown()
        sys.exit(1)
